...
exports.getIntegrationsStatus = async (req, res) => {
  const all = await getAllUserIntegrations(req.user.id);
  const out = {};
  for (const p of VALID_PROVIDERS) {
    const row = all.find((r) => r.provider === p);
    out[p] = shapeForFrontend(row);
  }
  res.json({ integrations: out, deliveries: await listDeliveries(req.user.id, 20) });
};
...
exports.upsertIntegration = async (req, res) => {
  try {
    ...
    const row = await setUserIntegration(req.user.id, provider, {
      apiKey: body.apiKey,
...
exports.deleteIntegration = async (req, res) => {
  ...
  const ok = await deleteUserIntegration(req.user.id, provider);
  res.json({ success: ok });
};
...
async function loadBriefForUser(briefId, userId) {
  const db = getDb();
  const { data: row, error } = await db
    .from("briefs")
    .select("id, user_id, client_name, project_name, original_text, completeness_score")
    .eq("id", briefId)
    .maybeSingle();

  if (!row) return { error: "not_found" };
  if (row.user_id !== userId) return { error: "forbidden" };
  return {
    id: row.id,
...
exports.exportToClickUp = async (req, res) => {
  try {
    const brief = await loadBriefForUser(req.params.id, req.user.id);
    ...
    const { data: stored, error } = await db
      ...
      .select("api_key, target_id")
      .eq("user_id", req.user.id)
      .eq("provider", "clickup")
      .maybeSingle();

    if (!stored || !stored.api_key || !stored.target_id) {
      return res.status(400).json({ error: "ClickUp is not configured. Add it in Integrations." });
    }
    const apiKey = unmaskSecret(stored.api_key);
    const listId = req.body?.listId || stored.target_id;
...
exports.exportToAirtable = async (req, res) => {
  try {
    ...
    const { data: stored, error } = await db
      .from("user_integrations")
      .select("api_key, target_id, target_id_2")
      .eq("user_id", req.user.id)
      .eq("provider", "airtable")
      .maybeSingle();

    if (!stored || !stored.api_key || !stored.target_id || !stored.target_id_2) {
      return res.status(400).json({ error: "Airtable is not configured. Add it in Integrations." });
    }
    const apiKey = unmaskSecret(stored.api_key);
    const baseId = req.body?.baseId || stored.target_id;
    const tableId = req.body?.tableId || stored.target_id_2;
    const improvedText = typeof req.body?.improvedText === "string" ? req.body.improvedText : null;
    const content = improvedText || brief.content;
...
exports.exportToNotionUser = async (req, res) => {
  try {
    ...
    const apiKey = unmaskSecret(stored.api_key);
    const databaseId = req.body?.databaseId || stored.target_id;
    const improvedText = typeof req.body?.improvedText === "string" ? req.body.improvedText : null;
    const content = improvedText || brief.content;
